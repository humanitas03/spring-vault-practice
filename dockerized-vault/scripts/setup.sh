...
## INIT VAULT
echo "[*] Init vault..."
#vault init -address=${VAULT_ADDR} > ./data/keys.txt
vault operator init -address=http://127.0.0.1:8200 > ./data/keys.txt
#export VAULT_TOKEN=$(grep 'Initial Root Token:' ./data/keys.txt | awk '{print substr($NF, 1, length($NF)-1)}')
export VAULT_TOKEN=$(grep 'Initial Root Token: ' ./data/keys.txt | awk '{print substr($NF, 0, 26)}')
